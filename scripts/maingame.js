...
if (canvas !== null)
    window.addEventListener('load', function () { grid = new ConwayGrid(canvas.clientWidth, canvas.clientHeight); grid.DrawGrid(); }, false);
...
if (slider !== null)
    slider.addEventListener('change', (e) => grid.UpdateSpeed());
class ConwayGrid {
    ...
    ClickGrid(event) {
        ...
        const x = Math.trunc((event.pageX - rect.left) / 5), y = Math.trunc((event.pageY - rect.top - window.scrollY) / 5);
    ...
    UpdateSpeed() {
        if (this.slider === null || this.output === null)
            return;
        console.log(this.slider.value);
        let val = 10 * (Number(this.slider.value) - 1);
        this.interval = val;

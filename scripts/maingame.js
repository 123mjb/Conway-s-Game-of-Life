...
class Grid {
    ...
    Run(){
        this.IntervalID = setInterval(() => {
            ...
            for (let i =0;i<this.x;i++){
                for (let j = 0; j < this.y; j++) {
                    let total = this.FindTotalSurrounmding(i,j);
                    if (total < 2) {
        ...
        // The above code runs every 10 milliseconds, you can adjust this value for speed
    }
    Stop(){
    ...
    FindTotalSurrounmding(i, j) {
        ...
        return total; // Subtract the current cell value
    }
    GetValue(x, y) {
